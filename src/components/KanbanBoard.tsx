...
export function KanbanBoard({ tasks, labels, onTasksChange }: KanbanBoardProps) {
  ...
  const archiveTask = (id: string) => {
    onTasksChange(
      tasks.map((task) =>
        task.id === id ? { ...task, columnId: 'archive' as ColumnId } : task
      )
    );
  };

  const editTask = (id: string, updates: Partial<Omit<KanbanTask, 'id' | 'createdAt' | 'columnId'>>) => {
    onTasksChange(
      tasks.map((task) =>
